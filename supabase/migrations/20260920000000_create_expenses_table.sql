...
-- Date: 2026-09-20
-- Description: Creates expenses table with org_id, created_by, category, amount, payment_method, RLS policies, and indexes.
...
-- RLS Policies
CREATE POLICY "Enable read access for expenses based on org_id" ON public.expenses
  FOR SELECT
  USING (true);

CREATE POLICY "Enable insert access for expenses based on org_id" ON public.expenses
  FOR INSERT
  WITH CHECK (true);

CREATE POLICY "Enable update access for expenses based on org_id" ON public.expenses
  FOR UPDATE
  USING (true);

CREATE POLICY "Enable delete access for expenses based on org_id" ON public.expenses
  FOR DELETE
  USING (true);
