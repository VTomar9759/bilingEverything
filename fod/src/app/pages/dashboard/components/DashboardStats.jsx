...
const DashboardStats = ({ stats = {} }) => {
  const statItems = [
    ...
    },
    {
      ...
      change: stats.lowStockCount > 0 ? "Requires immediate reorder" : "All stocks healthy",
      isPositive: stats.lowStockCount === 0,
      color: stats.lowStockCount > 0 ? "#ef4444" : "#10b981",
      bgGlow: stats.lowStockCount > 0 ? "rgba(239,68,68,0.06)" : "rgba(16,185,129,0.06)",
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={stats.lowStockCount > 0 ? "#ef4444" : "#10b981"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <StatsGrid>
      {statItems.map((item, idx) => (
        <StatCard key={idx} $glow={item.bgGlow} $color={item.color}>
          <IconContainer $glow={item.bgGlow}>
            {item.icon}
          </IconContainer>
          <StatContent>
            <Title>{item.title}</Title>
            <Value>{item.value}</Value>
            <Subtext $isPositive={item.isPositive} $isDanger={item.title === "Low Stock Items" && stats.lowStockCount > 0}>
              {item.change}
            </Subtext>
          </StatContent>
        </StatCard>
...
const Subtext = styled.span`
  ...
  color: ${({ $isPositive, $isDanger }) =>
    $isDanger ? "#ef4444" : $isPositive ? "var(--color-success)" : "var(--color-text-secondary)"};
`;
