  ...
  useEffect(() => {
    const fetchReportsData = async () => {
      ...
      try {
        ...
        // Compile payment channels
        const cashAmt = paidOrders.filter(o => o.payment_method === "Cash").reduce((acc, c) => acc + c.total, 0);
        const cardAmt = paidOrders.filter(o => o.payment_method === "Card").reduce((acc, c) => acc + c.total, 0);
        const upiAmt = paidOrders.filter(o => o.payment_method === "UPI").reduce((acc, c) => acc + c.total, 0);
        setPaymentBreakdown({ cashAmt, cardAmt, upiAmt });
    ...
    <PageWrapper>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Settle channels cards */}
          <PaymentsBreakdownCard>
            <TableTitle>
              <PieChartOutlined style={{ color: "var(--color-primary)" }} />
              <span>Settle Channel Breakdown</span>
            </TableTitle>
            <PaymentItemsGrid>
              <PaymentCol>
                <span>Cash Register POS</span>
                <strong>{currency} {paymentBreakdown.cashAmt?.toLocaleString()}</strong>
              </PaymentCol>
              <PaymentCol>
                <span>Credit/Debit Card</span>
                <strong>{currency} {paymentBreakdown.cardAmt?.toLocaleString()}</strong>
              </PaymentCol>
              <PaymentCol>
                <span>UPI Dynamic QR Scan</span>
                <strong>{currency} {paymentBreakdown.upiAmt?.toLocaleString()}</strong>
              </PaymentCol>
            </PaymentItemsGrid>
          </PaymentsBreakdownCard>
        </>
...
const SummaryCard = styled(SurfaceCard)`
  ...
  gap: 6px;
  span { font-size: 13px; color: var(--color-text-secondary); font-weight: 500; }
  strong { font-size: 20px; font-weight: 800; font-family: var(--font-display); color: var(--color-text-primary); }
`;
...
const PaymentsBreakdownCard = styled(TableCard)`
  width: 100%;
  margin-top: 10px;
`;

const PaymentItemsGrid = styled.div`
  display: grid;
...
const PaymentCol = styled.div`
  ...
  flex-direction: column;
  gap: 6px;
  span { font-size: 12px; color: var(--color-text-secondary); font-weight: 500; }
  strong { font-size: 18px; font-weight: 800; color: var(--color-primary-dark); }
`;
